...
pub fn update_right_panel(ctx: &egui::Context, app: &mut Monitor) {
    // both need to be *dereferenced to be used
    ...
    egui::SidePanel::right("left_right_panel")
        .show(ctx,|ui| {
            match *connected {
                ...
                false => {
                    ...
                    if ui.button("Connect").on_hover_text("Connect to YLab").clicked() {
                        *ylab_state = YLabState::Connected{version = *this_ylab, port = };
                    };
                }
            }
            ...
            egui::ComboBox::from_label("Serial Port")
                ...
                match *ylab_state {
                    YLabState::Disconnected => {
                        ui.label("Disconnected");
                    },
                    YLabState::Available {ports} => {
                        ui.label("Available");
                        egui::ComboBox::from_label("Serial Port")
                            .selected_text(format!("{}", ports.to_owned()))
                            .show_ui(ui, |ui| {
                                for i in ports.iter() {
                                    ui.selectable_value(&mut *i, 
                                                        i.to_string(), 
                                                        i.to_string());
                                }
                            });
                        if ui.button("Connect").on_hover_text("Connect to YLab").clicked() {
                            *ylab_state = YLabState::Connected{version: *ylab_state.version, port: *ylab_state.port}
                        };
                    },
        
        
                    YLabState::ConnRequest {version, port} => {
                        ui.label("Connecting");
                        ui.label(format!("{}:{}", version, port));
                    },
                    YLabState::Connected {version, port} => {
                        ui.label("Connected");
                        ui.label(format!("{}:{}", version, port));
                        if ui.button("Disconnect").on_hover_text("Disconnect from YLab").clicked(){
                            *connected = false;
                    ...
                    YLabState::Sending {version, port} => {
                        ui.label("Sending");
                        ui.label(format!("{}:{}", version, port));
                    },
                }
