...
pub struct Yui {
    ...
    pub selected_sensory: usize,
    pub selected_channels: [bool; 8],
    pub view: DataView,
    pub lowpass_threshold: f64,
    pub fft_min: usize,
    pub fft_max: usize, //opened_file: Option<PathBuf>,
    ...
    egui::SidePanel::right("left_right_panel").show(ctx, |ui| {
        ...
        match ylab_state.clone() {
            ...
            YLabState::Reading { version, port_name } => {
                ...
                if let Some(ytf_in) = ytf_wind.as_ref() {
                    if ytf_in.is_empty() {
                        ui.label(format!("No data from sensory"));
                        return; // very important! Otherwise the below can crash because of emtoy buffer
                    } 
                    
                } else {
                    panic!("No sensory histories available");
                }
                ...
                if n_banks > 1 {
                    ui.heading("Sensory");
                    ui.add(
                        egui::Slider::new(&mut ui_state.selected_sensory, 1..=(version.n_banks()))
                            .text(""),
                    );
                    ui.label(format!(
                        "Sensory {} of {}",
                        &ui_state.selected_sensory, n_banks
                    ));
                    ui.separator();
                }
                ...
                // Show running stats
                if ui_state.selected_sensory > 0 {
                    let buffer_size = ytf_wind.as_ref().unwrap().len();
                    let duration = ytf_wind.as_ref().unwrap().duration();
                    let sample_rate = buffer_size as f32 / duration;
                    let nyquist = (sample_rate / 2.) as f64;
                    let low_limit = (duration / 2.) as f64;
                    ui.separator();
                            ...
                            for (chan, active) in ui_state.selected_channels.iter().enumerate() {
                                ...
                                match coeffs {
                                    ...
                                    Ok(coeffs) => {
                                        ...
                                        // PLot the line
                                        let filtered_line = egui_plot::Line::new(PlotPoints::new(
                                            filtered_points.to_owned().into(),
                                        ))
